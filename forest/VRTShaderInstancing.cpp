...
{
	VRTShaderInstancing::VRTShaderInstancing(bool true_billboards, bool use_simple_normals) : m_TrueBillboards(true_billboards),
		m_PPL(true),
	...
	VRTShaderInstancing::~VRTShaderInstancing()
	{
	...
	osg::StateSet* VRTShaderInstancing::createStateSet(BillboardVegetationLayerVector &layers) 
	{
		...
		for(size_t i = 0; i < layers.size();i++)
		{
			if(image_map.find(layers[i].TextureName) == image_map.end() )
			{
				...
				layers[i]._TextureUnit = num_textures;
				num_textures++;
			}
			else
				layers[i]._TextureUnit = index_map[layers[i].TextureName];
	...
	osg::Geometry* VRTShaderInstancing::createSingleQuadsWithNormals( const osg::Vec3& pos, float w, float h)
	{
		// set up the coords
	...
	osg::Geometry* VRTShaderInstancing::createOrthogonalQuadsWithNormals( const osg::Vec3& pos, float w, float h)
	{
		// set up the coords
