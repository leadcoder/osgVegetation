...
struct ImpostorGenerator
{
	...
	void SetupStateSet(ImpostorModel im)
	{
		...
		fragmentShaderSource <<
			...
			"    vec3 pnormal = gl_NormalMatrix * normalize(ov_normal);\n"
			"    if(!(ov_normal.x == 0 && ov_normal.y == 0)) pnormal.y = 0;\n"
			...
			"    base_color.xyz = base_color.xyz*NdotL + base_color.xyz*gl_FrontLightModelProduct.sceneColor.xyz; \n"
