...
#pragma import_defines (FM_LINEAR,FM_EXP,FM_EXP2, BLT_ROTATED_QUAD)
...
#endif
	float NdotL = max(dot(normal, light_dir), 0); //normal.z*sqrt(ov_geometry_texcoord.y);// max(dot(normal, light_dir), 0);
