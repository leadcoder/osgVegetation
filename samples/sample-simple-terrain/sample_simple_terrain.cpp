...
int main(int argc, char** argv)
{
	...
	//We need to tell ov that tex unit 0 is used bu the color map of the terrain
	...
	osg::ref_ptr<osg::Group> vegetation = layer_generator.CreateVegetationNode(osgVegetation::CloneAndConvertToPatches(terrain, osg::CopyOp::DEEP_COPY_ALL));
	osg::ref_ptr<osg::StateSet>  terrain_ss = new osgVegetation::TerrainStateSet(osgVegetation::TerrainStateSetConfig());
	vegetation->getOrCreateStateSet()->merge(*terrain_ss);
	
	root_node->addChild(vegetation);
